...
Figma MCP не прочитал файл: `get_metadata` для `yuc5s9NCc4jENkk5LdFfvX` вернул отсутствие edit access. Пользователь выгрузил фото сам. В репозитории на шаге hero — `assets/images/img-hero.jpg`. Остальные файлы из той же выгрузки (слайдер, about, карточки) пока только на диске в `src/assets/`, не в этом шаге.
...
| --- | --- | --- |
| Изображения контента Home и Menu (hero, слайдер, остальные секции, карточки) | `assets/images/` | hero: `img-hero.jpg` (кадр `216:1370`). Слайдер, about и карточки — ещё не в git |
